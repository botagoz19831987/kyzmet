import {EQuestionType, ITest} from '../../../shared/models/test.model';

export const antiCorrupitonForHearOfEducation: ITest = {
    id: 'antiCorrupitonForHearOfEducation',
    title: 'O противодействии коррупции',
    description: '<p>Тест на знание закона «O противодействии коррупции»</p>',
    questions: [
        {
            type: EQuestionType.Radio,
            title: 'Лицо, постоянно, временно или по специальному полномочию осуществляющее функции представителя власти либо выполняющее организационно-распорядительные или административно-хозяйственные функции в государственных органах, субъектах квазигосударственного сектора, органах местного самоуправления, а также в Вооруженных Силах, других войсках и воинских формированиях: ',
            choices: [
                'должностное лицо',
                'лицо, занимающее ответственную государственную должность',
                'лицо, уполномоченное на выполнение государственных функций',
                'лицо, приравненное к лицам, уполномоченным на выполнение государственных функций'
            ],
            correctAnswer: 'должностное лицо'
        },
        {
            type: EQuestionType.Radio,
            title: 'Служащие Национального Банка и его ведомств: ',
            choices: [
                'лица, приравненные к лицам, уполномоченным на выполнение государственных функций',
                'лица, занимающие ответственную государственную должность',
                'должностные лица',
                'лица, уполномоченные на выполнение государственных функций'
            ],
            correctAnswer: 'лица, приравненные к лицам, уполномоченным на выполнение государственных функций'
        },
        {
            type: EQuestionType.Radio,
            title: 'Коррупционное правонарушение- это:',
            choices: [
                'имеющее признаки коррупции противоправное виновное деяние (действие или бездействие), за которое законом установлена административная или уголовная ответственность',
                'имеющее признаки коррупции противоправное виновное действие, за которое законом установлена уголовная ответственность',
                'имеющее признаки коррупции противоправное виновное бездействие, за которое законом установлена административная ответственность',
                'имеющее признаки коррупции противоправное виновное бездействие, за которое законом установлена дисциплинарная ответственность'
            ],
            correctAnswer: 'имеющее признаки коррупции противоправное виновное деяние (действие или бездействие), за которое законом установлена административная или уголовная ответственность'
        },
        {
            type: EQuestionType.Radio,
            title: 'Возможность возникновения причин и условий, способствующих совершению коррупционных правонарушений:',
            choices: [
                'коррупционный риск',
                'коррупционное правонарушение',
                'противодействие коррупции',
                'предупреждение коррупции'
            ],
            correctAnswer: 'коррупционный риск'
        },
        {
            type: EQuestionType.Radio,
            title: 'Основные принципы противодействия коррупции:',
            choices: [
                'законность, гласность, прозрачность, взаимодействие государства и гражданского общества',
                'равенство всех перед законом и судом, обеспечение государством социальной и правовой защиты государственных служащих',
                'принцип меритократии, профессионализм государственных служащих, казахстанский патриотизма',
                'этичность, нетерпимость к правонарушениям, равная оплата труда за выполнение равнозначной работы'
            ],
            correctAnswer: 'законность, гласность, прозрачность, взаимодействие государства и гражданского общества'
        },
        {
            type: EQuestionType.Radio,
            title: 'Противоречие между личными интересами и должностными полномочиями, при котором личные интересы могут привести к ненадлежащему исполнению должностных полномочий:',
            choices: [
                'конфликт интересов',
                'предупреждение коррупции',
                'коррупционное правонарушение',
                'коррупционный риск'
            ],
            correctAnswer: 'конфликт интересов'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не входит в систему мер противодействия коррупции:',
            choices: [
                'антикоррупционная политика',
                'анализ коррупционных рисков',
                'финансовый контроль',
                'сообщение о коррупционных правонарушениях'
            ],
            correctAnswer: 'антикоррупционная политика'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не входит в систему мер противодействия коррупции: ',
            choices: [
                'коррупционное правонарушение',
                'антикоррупционный мониторинг',
                'выявление коррупциогенных норм при производстве юридической экспертизы',
                'предотвращение и разрешение конфликта интересов'
            ],
            correctAnswer: 'коррупционное правонарушение'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционный мониторинг:',
            choices: [
                'деятельность субъектов противодействия коррупции по сбору, обработке, обобщению, анализу и оценке информации, касающейся эффективности антикоррупционной политики, состояния правоприменительной практики в сфере противодействия коррупции, а также восприятия и оценки уровня коррупции обществом',
                'выявление и изучение причин и условий, способствующих совершению коррупционных правонарушений',
                'установленная для обособленной сферы общественных отношений система рекомендаций, направленная на предупреждение коррупции',
                'документ, содержащий анализ и оценку состояния и тенденции распространения коррупции на международном и национальном уровнях, предложения по формированию, реализации и совершенствованию антикоррупционной политики'
            ],
            correctAnswer: 'деятельность субъектов противодействия коррупции по сбору, обработке, обобщению, анализу и оценке информации, касающейся эффективности антикоррупционной политики, состояния правоприменительной практики в сфере противодействия коррупции, а также восприятия и оценки уровня коррупции обществом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Цель антикоррупционного мониторинга:',
            choices: [
                'оценка правоприменительной практики в сфере противодействия коррупции',
                'антикоррупционное образование',
                'выявление коррупциогенных норм при производстве юридической экспертизы',
                'выявление коррупционных рисков в нормативных правовых актах, затрагивающих деятельность государственных органов и организаций, субъектов квазигосударственного сектора'
            ],
            correctAnswer: 'оценка правоприменительной практики в сфере противодействия коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Анализ коррупционных рисков предусматривает:',
            choices: [
                'выявление и изучение причин и условий, способствующих совершению коррупционных правонарушений',
                'оценку правоприменительной практики в сфере противодействия коррупции',
                'выявление коррупциогенных норм при производстве юридической экспертизы',
                'систему рекомендаций, направленная на предупреждение коррупции'
            ],
            correctAnswer: 'выявление и изучение причин и условий, способствующих совершению коррупционных правонарушений'
        },
        {
            type: EQuestionType.Radio,
            title: 'Внешний анализ коррупционных рисков осуществляется:',
            choices: [
                'уполномоченным органом по противодействию коррупции',
                'государственными органами, организациями и субъектами квазигосударственного сектора',
                'Правительством',
                'правоохранительными органами'
            ],
            correctAnswer: 'уполномоченным органом по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Внутренний анализ коррупционных рисков осуществляется:',
            choices: [
                'государственными органами, организациями и субъектами квазигосударственного сектора',
                'только государственными органами и организациями',
                'только субъектами квазигосударственного сектора',
                'Правительством'
            ],
            correctAnswer: 'государственными органами, организациями и субъектами квазигосударственного сектора'
        },
        {
            type: EQuestionType.Radio,
            title: 'Формирование антикоррупционной культуры:',
            choices: [
                'деятельность, осуществляемая субъектами противодействия коррупции в пределах своей компетенции по сохранению и укреплению в обществе системы ценностей, отражающей нетерпимость к коррупции',
                'непрерывный процесс воспитания и обучения, осуществляемый в целях нравственного, интеллектуального, культурного развития и формирования активной гражданской позиции неприятия коррупции личностью',
                'разъяснительная работа в средствах массовой информации, организация социально значимых мероприятий',
                'правовые, административные и организационные меры, направленные на снижение коррупционных рисков, повышение доверия общества к деятельности государственных органов'
            ],
            correctAnswer: 'деятельность, осуществляемая субъектами противодействия коррупции в пределах своей компетенции по сохранению и укреплению в обществе системы ценностей, отражающей нетерпимость к коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Под антикоррупционным образованием понимается:',
            choices: [
                'непрерывный процесс воспитания и обучения, осуществляемый в целях нравственного, интеллектуального, культурного развития и формирования активной гражданской позиции неприятия коррупции личностью',
                'деятельность субъектов противодействия коррупции по изучению, выявлению, ограничению и устранению причин и условий, способствующих совершению коррупционных правонарушений',
                'деятельность субъектов противодействия коррупции по сбору, обработке, обобщению, анализу и оценке информации, касающейся эффективности антикоррупционной политики',
                'процесс формирования и соблюдения антикоррупционных стандартов'
            ],
            correctAnswer: 'непрерывный процесс воспитания и обучения, осуществляемый в целях нравственного, интеллектуального, культурного развития и формирования активной гражданской позиции неприятия коррупции личностью'
        },
        {
            type: EQuestionType.Radio,
            title: 'Формирование антикоррупционной культуры осуществляется посредством:',
            choices: [
                'комплекса мер образовательного, информационного и организационного характера',
                'проведения досудебного расследования, в том числе предварительного следствия, дознания и протокольной формы коррупционных преступлений',
                'реализации государственной политики в сфере безопасности граждан',
                'принятия мер по предотвращению и разрешению конфликта интересов'
            ],
            correctAnswer: 'комплекса мер образовательного, информационного и организационного характера'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционные стандарты:',
            choices: [
                'установленная для обособленной сферы общественных отношений система рекомендаций, направленная на предупреждение коррупции',
                'система выявления и изучения причин и условий, способствующих совершению коррупционных правонарушений',
                'система рекомендаций по принятию мер по предотвращению и разрешению конфликта интересов',
                'требования по устранению причин и условий, способствующих совершению коррупционных правонарушений'
            ],
            correctAnswer: 'установленная для обособленной сферы общественных отношений система рекомендаций, направленная на предупреждение коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'В целях недопущения совершения действий, которые могут привести к использованию своих полномочий в личных, групповых и иных неслужебных интересах, лица, занимающие ответственную государственную должность:',
            choices: [
                'принимают на себя антикоррупционные ограничения',
                'принимают ограничения по выезду за пределы страны',
                'принимают ограничения по выступлению в средствах массовой информации',
                'соблюдают принципы добросовестной конкуренции'
            ],
            correctAnswer: 'принимают на себя антикоррупционные ограничения'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не относится к антикоррупционным ограничениям:',
            choices: [
                'осуществление деятельности в пределах должностных полномочий',
                'принятие подарков в связи с исполнением служебных полномочий',
                'использование служебной и иной информации, не подлежащей официальному распространению, в целях получения или извлечения имущественных и неимущественных благ и преимуществ',
                'осуществление деятельности, не совместимой с выполнением государственных функций'
            ],
            correctAnswer: 'осуществление деятельности в пределах должностных полномочий'
        },
        {
            type: EQuestionType.Radio,
            title: 'Согласие на принятие антикоррупционных ограничений лиц, уполномоченных на выполнение государственных функций:',
            choices: [
                'фиксируется службами управления персоналом (кадровыми службами) соответствующих организаций в письменной форме',
                'фиксируется службами внутреннего контроля (внутренней безопасности) соответствующих организаций в письменной форме',
                'фиксируется юридическими службами соответствующих организаций в письменной форме',
                'фиксируется службами управления персоналом (кадровыми службами) соответствующих организаций в трудовом договоре'
            ],
            correctAnswer: 'фиксируется службами управления персоналом (кадровыми службами) соответствующих организаций в письменной форме'
        },
        {
            type: EQuestionType.Radio,
            title: 'Отказ в принятии антикоррупционных ограничений влечет:',
            choices: [
                'отказ в приеме на должность либо увольнение с должности (освобождение от должности)',
                'предупреждение о неполном служебном соответствии',
                'объявление выговора',
                'понижение в должности'
            ],
            correctAnswer: 'отказ в приеме на должность либо увольнение с должности (освобождение от должности)'
        },
        {
            type: EQuestionType.Radio,
            title: 'Несоблюдение антикоррупционных ограничений в случаях отсутствия признаков уголовно наказуемого деяния и административного правонарушения является основанием для:',
            choices: [
                'прекращения государственной службы или иной соответствующей деятельности',
                'перевода государственного служащего на иную должность',
                'наложения дисциплинарного взыскания на государственного служащего',
                'понижения государственного служащего в должности'
            ],
            correctAnswer: 'прекращения государственной службы или иной соответствующей деятельности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, занимающие ответственную государственную должность вправе: ',
            choices: [
                'сдавать в имущественный наем (аренду) жилище, принадлежащее им на праве собственности, и получать доход от такой сдачи',
                'предпринимательской деятельностью',
                'иной оплачиваемой должностью',
                'самостоятельно участвовать в управлении хозяйствующим субъектом, если управление или участие в управлении хозяйствующим субъектом не входит в их должностные обязанности'
            ],
            correctAnswer: 'сдавать в имущественный наем (аренду) жилище, принадлежащее им на праве собственности, и получать доход от такой сдачи'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не вправе приобретать паи инвестиционных фондов, облигации, акции коммерческих организаций:',
            choices: [
                'Председатель Национального банка и его заместителям',
                'лица, занимающие ответственную государственную должность',
                'акимы городов районного значения, поселков, сел, сельских округов',
                'лица, исполняющие управленческие функции в субъекте квазигосударственного сектора'
            ],
            correctAnswer: 'Председатель Национального банка и его заместителям'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, занимающие ответственную государственную должность, обязаны передать в доверительное управление принадлежащее им имущество: ',
            choices: [
                'в течение 30 календарных дней со дня вступления в должность',
                'в течение десяти рабочих дней со дня вступления в должность',
                'в течение 15 календарных дней со дня вступления в должность',
                'в течение 25 календарных дней со дня вступления в должность'
            ],
            correctAnswer: 'в течение 30 календарных дней со дня вступления в должность'
        },
        {
            type: EQuestionType.Radio,
            title: 'Договор на доверительное управление имуществом:',
            choices: [
                'подлежит нотариальному удостоверению',
                'составляется в простой форме, не подлежит нотариальному удостоверению',
                'подлежит нотариальному удостоверению только при передаче в доверительное управление паев инвестиционных фондов, облигаций и акций коммерческих организаций',
                'подлежит нотариальному удостоверению только при сдаче в имущественный наем (аренду)жилища'
            ],
            correctAnswer: 'подлежит нотариальному удостоверению'
        },
        {
            type: EQuestionType.Radio,
            title: 'Неисполнение обязательств по нотариальному удостоверению договора на доверительное управление имуществом:',
            choices: [
                'является основанием для прекращения ими государственной службы или иной соответствующей деятельности',
                'влечет дисциплинарное взыскание в виде выговора',
                'уголовной ответственностью',
                'основанием для понижения в должности'
            ],
            correctAnswer: 'является основанием для прекращения ими государственной службы или иной соответствующей деятельности'
        },
        {
            type: EQuestionType.Radio,
            title: 'В отношении лиц, нарушающих требования по недопущению совместной службы (работы) близких родственников, супругов или свойственников, если они добровольно не устранят нарушение, принимаются меры по:',
            choices: [
                'переводу на должности, исключающему такую подчиненность, а при невозможности такого перевода один из этих служащих подлежит увольнению с должности или иному освобождению от указанных функций',
                'обязательному освобождению лица от должности',
                'привлечению лица к дисциплинарной ответственности',
                'привлечению лица к административной ответственности'
            ],
            correctAnswer: 'переводу на должности, исключающему такую подчиненность, а при невозможности такого перевода один из этих служащих подлежит увольнению с должности или иному освобождению от указанных функций'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, занимающие ответственную государственную должность, лица, уполномоченные на выполнение государственных функций, лица, приравненные к лицам, уполномоченным на выполнение государственных функций, должностные лица при возниконовении конфликта интересов обязаны:',
            choices: [
                'в письменной форме уведомить непосредственного руководителя либо руководство организации',
                'изменить должностные обязанности',
                'принять меры по увольнению',
                'в любой форме уведомить непосредственного руководителя либо руководство организации'
            ],
            correctAnswer: 'в письменной форме уведомить непосредственного руководителя либо руководство организации'
        },
        {
            type: EQuestionType.Radio,
            title: 'Непосредственный руководитель либо руководство организации в целях предотвращения и урегулирования конфликта интересов, принимают меры по:',
            choices: [
                'отстранению лица от исполнения должностных обязанностей и поручению другому лицу исполнение должностных обязанностей по вопросу, в связи с которым возник или может возникнуть конфликт интересов',
                'наложению на государственного служащего дисциплинарного взыскания',
                'предупреждению лица о неполном служебном соответствии или понижению в государственной должности',
                'привлечению лица к административной ответственности'
            ],
            correctAnswer: 'отстранению лица от исполнения должностных обязанностей и поручению другому лицу исполнение должностных обязанностей по вопросу, в связи с которым возник или может возникнуть конфликт интересов'
        },
        {
            type: EQuestionType.Radio,
            title: 'Национальный доклад о противодействии коррупции:',
            choices: [
                'документ, содержащий анализ и оценку состояния и тенденции распространения коррупции на международном и национальном уровнях, предложения по формированию, реализации и совершенствованию антикоррупционной политики',
                'документ, содержащий оценку правоприменительной практики в сфере государственной службы и противодействия коррупции',
                'документ, устанавливающий систему рекомендаций, направленную на предупреждение коррупции',
                'нормативный правовой акт, разрабатываемый уполномоченным органом по противодействию коррупции и утверждаемый Президентом'
            ],
            correctAnswer: 'документ, содержащий анализ и оценку состояния и тенденции распространения коррупции на международном и национальном уровнях, предложения по формированию, реализации и совершенствованию антикоррупционной политики'
        },
        {
            type: EQuestionType.Radio,
            title: 'Национальный доклад о противодействии коррупции формируется: ',
            choices: [
                'ежегодно',
                'дважды в год',
                'один раз в три года',
                'один раз в два года'
            ],
            correctAnswer: 'ежегодно'
        },
        {
            type: EQuestionType.Radio,
            title: 'Информация о лице, оказывающем содействие в противодействии коррупции, является:',
            choices: [
                'государственным секретом',
                'государственной тайной',
                'информацией для служебного пользования',
                'общедоступной информацией'
            ],
            correctAnswer: 'государственным секретом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицо, сообщившее о факте коррупционного правонарушения или иным образом оказывающее содействие в противодействии коррупции:',
            choices: [
                'находится под защитой государства',
                'проверяется на факт причастности к коррупционному правонарушению',
                'обеспечивается личной охраной до выяснения обстоятельств своершенного коррупционного правонарушения',
                'находится под защитой правоохранительных органов'
            ],
            correctAnswer: 'находится под защитой государства'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицо, располагающее информацией о коррупционном правонарушении, уведомляет об этом: ',
            choices: [
                'руководство государственного органа либо организации, сотрудником которой является, либо уполномоченный орган по противодействию коррупции',
                'сотрудника структурного подразделения, в котором он работает',
                'вышестоящий государственный орган',
                'любой правоохранительный орган'
            ],
            correctAnswer: 'руководство государственного органа либо организации, сотрудником которой является, либо уполномоченный орган по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Меры противодействия коррупции, принимаемые физическими лицами, общественными объединениями и иными юридическими лицами:',
            choices: [
                'сообщение об известных им фактах совершения коррупционных правонарушений',
                'изучение и распространение положительного опыта противодействия коррупции',
                'запрос у государственных органов, организаций и должностных лиц информации и материалов, способствующих раскрытию коррупционных правонарушений',
                'проведение анализа практики оперативно-розыскной и следственной деятельности, досудебного расследования по коррупционным преступлениям'
            ],
            correctAnswer: 'сообщение об известных им фактах совершения коррупционных правонарушений'
        },
        {
            type: EQuestionType.Radio,
            title: 'Выявление, пресечение, раскрытие, расследование и предупреждение коррупционных правонарушений и привлечение лиц, виновных в их совершении, к ответственности в пределах компетенции осуществляется:',
            choices: [
                'органами прокуратуры, национальной безопасности, внутренних дел, военной полиции, службой экономических расследований, Пограничной службой КНБ',
                'государственными органами, организациями, субъектами квазигосударственного сектора и должностными лицами',
                'органами прокуратуры, национальной безопасности, внутренних дел',
                'органами прокуратуры, Пограничной службой Комитета национальной безопасности'
            ],
            correctAnswer: 'органами прокуратуры, национальной безопасности, внутренних дел, военной полиции, службой экономических расследований, Пограничной службой КНБ'
        },
        {
            type: EQuestionType.Radio,
            title: 'К субъектам противодействия коррупции не относятся:',
            choices: [
                'международные неправительственные организации',
                'субъекты квазигосударственного сектора',
                'государственные органы',
                'общественные объединения, физические и юридические лица'
            ],
            correctAnswer: 'международные неправительственные организации'
        },
        {
            type: EQuestionType.Radio,
            title: 'Функцией уполномоченного органа по противодействию коррупции является:',
            choices: [
                'разработка предложений по совершенствованию нормативной правовой базы в сфере противодействия коррупции, а также принятие нормативных правовых актов в пределах своей компетенции',
                'внесение предложений по формированию государственной политики в сферах профилактики правонарушений, охраны общественного порядка, борьбы с преступностью, исполнения уголовных наказаний и административных взысканий',
                'обеспечение оперативного управления силами и средствами при совершении тяжких преступлений, массовых беспорядках, стихийных бедствиях и других чрезвычайных происшествиях',
                'создание и функционирование Республиканской информационной системы в области охраны общественного порядка и борьбы с преступностью'
            ],
            correctAnswer: 'разработка предложений по совершенствованию нормативной правовой базы в сфере противодействия коррупции, а также принятие нормативных правовых актов в пределах своей компетенции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционная служба уполномоченного органа по противодействию коррупции в пределах своих полномочий вправе:',
            choices: [
                'проводить анализ практики оперативно-розыскной и следственной деятельности, досудебного расследования по коррупционным преступлениям',
                'обеспечивать реализацию государственной политики в сфере оборота наркотических средств, психотропных веществ и прекурсоров, противодействие их незаконному обороту и злоупотреблению ими',
                'организовать специальные и военные перевозки',
                'обеспечивать создание и функционирование Республиканской информационной системы в области охраны общественного порядка и борьбы с преступностью'
            ],
            correctAnswer: 'проводить анализ практики оперативно-розыскной и следственной деятельности, досудебного расследования по коррупционным преступлениям'
        },
        {
            type: EQuestionType.Radio,
            title: 'Взыскание незаконно полученного имущества или оплаты государству его стоимости или стоимости незаконно полученных услуг в результате коррупционных правонарушений в случаях отказа их добровольной сдачи производится на основании:',
            choices: [
                'вступившего в законную силу решения суда по иску прокурора, органов государственных доходов либо других государственных органов и должностных лиц, уполномоченных на это законом',
                'иска прокурора, органов государственных доходов либо других государственных органов и должностных лиц, уполномоченных на это законом',
                'решения суда по иску прокурора, или иного субъекта противодействия коррупции',
                'решения суда, вступившего в законную силу, по иску физических лиц'
            ],
            correctAnswer: 'вступившего в законную силу решения суда по иску прокурора, органов государственных доходов либо других государственных органов и должностных лиц, уполномоченных на это законом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сделки, договоры, совершенные в результате коррупционных правонарушений, признаются недействительными:',
            choices: [
                'судом',
                'актом прокурорского надзора',
                'антикоррупционной службой',
                'уполномоченным органом по противодействию коррупции'
            ],
            correctAnswer: 'судом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, исполняющие управленческие функции в субъектах квазигосударственного сектора:',
            choices: [
                'лица, постоянно, временно либо по специальному полномочию исполняющие организационно-распорядительные или административно-хозяйственные функции в указанных организациях',
                'лица, постоянно исполняющие организационно-распорядительные функции',
                'государственные служащие',
                'лица, занимающие политические государственные должности либо административные государственный должности корпуса «А»'
            ],
            correctAnswer: 'лица, постоянно, временно либо по специальному полномочию исполняющие организационно-распорядительные или административно-хозяйственные функции в указанных организациях'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционная политика – это:',
            choices: [
                'правовые, административные и организационные меры, направленные на снижение коррупционных рисков, повышение доверия общества к деятельности государственных органов, и иные меры',
                'деятельность субъектов противодействия коррупции по предупреждению коррупции, в том числе по формированию антикоррупционной культуры в обществе, выявлению и устранению причин и условий, способствующих совершению коррупционных правонарушений',
                'деятельность субъектов противодействия коррупции по изучению, выявлению, ограничению и устранению причин и условий, способствующих совершению коррупционных правонарушений, путем разработки и внедрения системы превентивных мер',
                'возможность возникновения причин и условий, способствующих совершению коррупционных правонарушений'
            ],
            correctAnswer: 'правовые, административные и организационные меры, направленные на снижение коррупционных рисков, повышение доверия общества к деятельности государственных органов, и иные меры'
        },
        {
            type: EQuestionType.Radio,
            title: 'К мерам, принимаемым непосредственным руководителем либо руководством организации по урегулированию конфликта интересов не относится:',
            choices: [
                'незамедлительное увольнение лица с занимаемой должности',
                'отстранение лиц, в отношении которых возник конфликт интересов, от исполнения должностных обязанностей и поручить другому лицу исполнение должностных обязанностей по вопросу, в связи с которым возник или может возникнуть конфликт интересов',
                'изменение должностных обязанностей',
                'принятие иных мер по устранению конфликта интересов'
            ],
            correctAnswer: 'незамедлительное увольнение лица с занимаемой должности'
        },
        {
            type: EQuestionType.Radio,
            title: 'За коррупционное правонарушение лицо привлекается к:',
            choices: [
                'административной и уголовной ответственности',
                'дисциплинарной и административной ответственности',
                'гражданско-правовой и уголовной ответственности',
                'дисциплинарной и гражданско-правовой ответственности'
            ],
            correctAnswer: 'административной и уголовной ответственности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, уволенные с государственной службы по отрицательным мотивам, представляют в орган государственных доходов по месту жительства декларацию о доходах и имуществе в течение:',
            choices: [
                'трех лет после увольнения в порядке, установленном налоговым законодательством',
                'шести месяцев после увольнения в порядке, установленном налоговым законодательством',
                'одного года после увольнения в порядке, установленном налоговым законодательством',
                'двух лет после увольнения в порядке, установленном налоговым законодательством'
            ],
            correctAnswer: 'трех лет после увольнения в порядке, установленном налоговым законодательством'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицам, уполномоченным на выполнение государственных функций, и лицам, приравненным к ним, запрещается:',
            choices: [
                'заключать гражданско-правовые сделки не под своим именем',
                'сдавать в имущественный наем (аренду) жилище, принадлежащее им на праве собственности, и получать доход от такой сдачи',
                'заниматься оплачиваемой педагогической деятельностью',
                'самостоятельно участвовать в управлении хозяйствующим субъектом, если управление или участие в управлении хозяйствующим субъектом входит в их должностные обязанности'
            ],
            correctAnswer: 'заключать гражданско-правовые сделки не под своим именем'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, сообщившие заведомо ложную информацию о факте коррупционного правонарушения:',
            choices: [
                'подлежат ответственности в соответствии с законом',
                'находятся под защитой государства',
                'привлекаются к дисциплинарной ответственности по усмотрению руководителя антикоррупционной службы',
                'подлежат увольнению с занимаемой должности'
            ],
            correctAnswer: 'подлежат ответственности в соответствии с законом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, занимающие государственную должность, в период выполнения своих полномочий представляют в орган государственных доходов декларацию о доходах и имуществе:',
            choices: [
                'ежегодно',
                'по итогам полугодий',
                'ежемесячно',
                'ежеквартально'
            ],
            correctAnswer: 'ежегодно'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сведения о размерах и об источниках доходов должностных лиц, занимающих ответственные государственные должности:',
            choices: [
                'могут быть опубликованы в порядке, установленном законодательством',
                'составляют служебную тайну',
                'являются государственным секретом',
                'не подлежат разглашению'
            ],
            correctAnswer: 'могут быть опубликованы в порядке, установленном законодательством'
        },
        {
            type: EQuestionType.Radio,
            title: 'Результаты антикоррупционного мониторинга могут являться основанием для: ',
            choices: [
                'проведения анализа коррупционных рисков, а также совершенствование мер, направленных на формирование антикоррупционной культуры',
                'расследования по факту совершения коррупционного правонарушения',
                'освобождения лица от государственной должности в соответствии с законом',
                'признания нормативного правового акта противоречащим Конституции'
            ],
            correctAnswer: 'проведения анализа коррупционных рисков, а также совершенствование мер, направленных на формирование антикоррупционной культуры'
        },
        {
            type: EQuestionType.Radio,
            title: 'Согласно Закону «О противодействии коррупции» к близким родственникам не относятся:',
            choices: [
                'супруг (супруга)',
                'родители (родитель), усыновители (удочерители)',
                'дети, усыновленные (удочеренные)',
                'полнородные и неполнородные братья и сестры'
            ],
            correctAnswer: 'супруг (супруга)'
        },
        {
            type: EQuestionType.Radio,
            title: 'Принятие актов и совершение действий в результате коррупционных правонарушений являются основаниями:',
            choices: [
                'для их отмены (признания недействительными)',
                'их переутверждения',
                'привлечения лица к уголовной ответственности',
                'начала расследования в отношении лиц, принявших акт'
            ],
            correctAnswer: 'для их отмены (признания недействительными)'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицо, исполняющее управленческие функции в государственной организации или субъекте квазигосударственного сектора, относится к категории лиц:',
            choices: [
                'приравненных к лицам, уполномоченным на выполнение государственных функций',
                'должностных лиц',
                'уполномоченных на выполнение государственных функций',
                'занимающих ответственную государственную должность'
            ],
            correctAnswer: 'приравненных к лицам, уполномоченным на выполнение государственных функций'
        },
        {
            type: EQuestionType.Radio,
            title: 'Меры финансового контроля, предусмотренные Законом «О противодействии коррупции» НЕ распространяются на правоотношения, связанные (вопрос по старой редакции):',
            choices: [
                'с приобретением в собственность жилищ и строительных материалов для строительства жилищ в Республике Казахстан',
                'с участием лиц в качестве акционера или учредителя (участника) юридических лиц',
                'с приобретением автотранспорта',
                'с обязательствами (в том числе и устными) по содержанию или временному хранению материальных и финансовых средств, принадлежащих лицу или супругу (супруге)'
            ],
            correctAnswer: 'с приобретением в собственность жилищ и строительных материалов для строительства жилищ в Республике Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'Уголовная ответственность и наказание за коррупционные преступления предусматривается:',
            choices: [
                'Уголовным кодексом',
                'Законом «О противодействии коррупции»',
                'Уголовно-процессуальным кодексом',
                'Кодексом об административных правонарушениях'
            ],
            correctAnswer: 'Уголовным кодексом'
        },
        {
            type: EQuestionType.Radio,
            title: 'К источникам антикоррупционного мониторинга не относятся:',
            choices: [
                'сведения, составляющие государственную тайну',
                'правовая статистика и обращения физических и юридических лиц',
                'сведения неправительственных и международных организаций',
                'данные социологических опросов и публикаций в средствах массовой информации, а также иные не запрещенные законом источники информации'
            ],
            correctAnswer: 'сведения, составляющие государственную тайну'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сведения о доходах кандидатов на выборные государственные должности при их выдвижении:',
            choices: [
                'могут быть опубликованы в порядке, установленном законодательством',
                'составляют служебную тайну',
                'являются государственным секретом',
                'не подлежат разглашению'
            ],
            correctAnswer: 'могут быть опубликованы в порядке, установленном законодательством'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не относится к системе мер противодействия коррупции:',
            choices: [
                'развитие международного сотрудничества по противодействию коррупции',
                'антикоррупционный мониторинг',
                'анализ коррупционных рисков',
                'формирование антикоррупционной культуры'
            ],
            correctAnswer: 'развитие международного сотрудничества по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Противодействие коррупции в пределах своей компетенции обязаны вести:',
            choices: [
                'все государственные органы, организации, субъекты квазигосударственного сектора и должностные лица',
                'специальные органы',
                'физические и юридические лица',
                'только правоохранительные органы'
            ],
            correctAnswer: 'все государственные органы, организации, субъекты квазигосударственного сектора и должностные лица'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не являются, приравненными к лицам, уполномоченным на выполнение государственных функций, относятся?',
            choices: [
                'преподаватели государственных вузов',
                'лицо, избранное в органы местного самоуправления',
                'гражданин, зарегистрированный в качестве кандидата в Президенты',
                'лицо, исполняющее управленческие функции в государственной организации или субъекте квазигосударственного сектора'
            ],
            correctAnswer: 'преподаватели государственных вузов'
        },
        {
            type: EQuestionType.Radio,
            title: 'Привлечение лиц, виновных в совершении коррупционных правонарушений, к ответственности в пределах своей компетенции не осуществляется?',
            choices: [
                'органами прокуратуры',
                'общественными советами',
                'органами национальной безопасности',
                'службой экономических расследований'
            ],
            correctAnswer: 'общественными советами'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не является принципом противодействия коррупции?',
            choices: [
                'законности',
                'приоритета защиты прав, свобод и законных интересов человека и гражданина',
                'независимость от деятельности политических партий, общественных объединений',
                'гласности и прозрачности'
            ],
            correctAnswer: 'независимость от деятельности политических партий, общественных объединений'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не является принципом противодействия коррупции?',
            choices: [
                'тотальный антикоррупционный контроль',
                'взаимодействия государства и гражданского общества',
                'поощрения лиц, оказывающих содействие в противодействии коррупции',
                'неотвратимости наказания за совершение коррупционных правонарушений'
            ],
            correctAnswer: 'тотальный антикоррупционный контроль'
        },
        {
            type: EQuestionType.Radio,
            title: 'Кто определяет Порядок проведения внешнего анализа коррупционных рисков?',
            choices: [
                'Правительство Республики Казахстан',
                'Агентство по делам государственной службы',
                'Премьер-Министр',
                'Уполномоченный орган по противодействию коррупции'
            ],
            correctAnswer: 'Правительство Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'Субъекты предпринимательства при осуществлении своей деятельности принимают меры по?',
            choices: [
                'принятию и соблюдению норм корректного взаимодействия',
                'предотвращению конфликта интересов',
                'ведению антикоррупционного мониторинга',
                'соблюдению принципов недобросовестной конкуренции'
            ],
            correctAnswer: 'предотвращению конфликта интересов'
        },
        {
            type: EQuestionType.Radio,
            title: 'К субъектам противодействия коррупции относятся?',
            choices: [
                'государственные организации, субъекты государственного сектора',
                'средства массовой информации',
                'только физические и юридические лица',
                'государственные организации и средства массовой информации'
            ],
            correctAnswer: 'государственные организации, субъекты государственного сектора'
        },
        {
            type: EQuestionType.Radio,
            title: 'Внешний анализ коррупционных рисков осуществляется уполномоченным органом по противодействию коррупции в порядке, определяемом кем?',
            choices: [
                'Президентом',
                'Парламентом',
                'Уполномоченным органом по противодействию коррупции',
                'Правительством по согласованию с Администрацией Президента'
            ],
            correctAnswer: 'Правительством по согласованию с Администрацией Президента'
        },
        {
            type: EQuestionType.Radio,
            title: 'Со дня назначения на должность Председатель Национального Банка и его заместители обязаны передать в доверительное управление принадлежащие облигации и акции коммерческих организаций, приобретённые до назначения их на должность в какой период?',
            choices: [
                'в течении 30 часов',
                'в течении 30 дней',
                'в течении 30 календарных дней',
                'в течении 30 рабочих дней'
            ],
            correctAnswer: 'в течении 30 календарных дней'
        },
        {
            type: EQuestionType.Radio,
            title: 'Относится к направлениям внешнего анализа коррупционных рисков?',
            choices: [
                'анализ оперативно-розыскной деятельности',
                'выявление коррупционных рисков в нормативных правовых актах, затрагивающих деятельность государственных органов и организаций, субъектов квазигосударственного сектора',
                'контроль высшего надзора, осуществляемого прокуратурой',
                'анализ производства по делам об административных правонарушениях'
            ],
            correctAnswer: 'выявление коррупционных рисков в нормативных правовых актах, затрагивающих деятельность государственных органов и организаций, субъектов квазигосударственного сектора'
        },
        {
            type: EQuestionType.Radio,
            title: 'Какие сведения из деклараций физических лиц не подлежат опубликованию на официальном интернет-ресурсе государственного органа, осуществляющего руководство в сфере обеспечения поступлений налогов и других обязательных платежей в бюджет?',
            choices: [
                'сведения, составляющие государственный секрет',
                'сведения о доходах',
                'сведения об имуществе',
                'сведения о пенсионных отчислениях'
            ],
            correctAnswer: 'сведения, составляющие государственный секрет'
        },
        {
            type: EQuestionType.Radio,
            title: 'Что такое предупреждение коррупции?',
            choices: [
                'деятельность субъектов противодействия коррупции по изучению, выявлению, ограничению, устранению причин и условий, способствующих совершению коррупционных правонарушений, путём разработки и внедрения системы превентивных мер',
                'правовые, административные и организационные меры, направленные на снижение коррупционных рисков, повышение доверия общества к деятельности государственных органов, и иные меры в соответствии с настоящим Законом',
                'имеющее признаки коррупции противоправное виновное деяние (действие или бездействие), за которое законом установлена административная или уголовная ответственность',
                'возможность возникновения причин и условий, способствующих совершению коррупционных правонарушений'
            ],
            correctAnswer: 'деятельность субъектов противодействия коррупции по изучению, выявлению, ограничению, устранению причин и условий, способствующих совершению коррупционных правонарушений, путём разработки и внедрения системы превентивных мер'
        },
        {
            type: EQuestionType.Radio,
            title: 'Физические лица, общественные объединения и иные юридические лица при противодействии коррупции?',
            choices: [
                'осуществляют взаимодействие с другими субъектами противодействия коррупции уполномоченным органом по противодействию коррупции',
                'проводят расследования по вопросам противодействия коррупции',
                'совершенствуют формы и методы борьбы с коррупционными преступлениями и правонарушениями, определяют стратегию и тактику оперативно-розыскной деятельности, вырабатывают и реализовывают меры по повышению ее эффективности',
                'выносят приговоры в отношении лиц, совершивших коррупционное правонарушение'
            ],
            correctAnswer: 'осуществляют взаимодействие с другими субъектами противодействия коррупции уполномоченным органом по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Одной из мер, применяемой, физическими лицами, общественными объединениями и иными юридическими лицами при противодействии коррупции является:',
            choices: [
                'запрашивают и получают в порядке, установленном законодательством Республики Казахстан, от специальных органов уголовные и административные дела',
                'проводят исследования, в том числе научные и социологические, по вопросам противодействия коррупции',
                'по имеющимся в производстве уголовным делам подвергают приводу лиц, уклоняющихся от явки по вызову',
                'выносят приговоры в отношении лиц, совершивших коррупционное правонарушение'
            ],
            correctAnswer: 'проводят исследования, в том числе научные и социологические, по вопросам противодействия коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Физические лица, общественные объединения и иные юридические лица при противодействии коррупции НЕ применяют следующие меры:',
            choices: [
                'вносят предложения по совершенствованию законодательства и правоприменительной практики по вопросам противодействия коррупции',
                'проводят расследования по вопросам противодействия коррупции',
                'запрашивают и получают в порядке, установленном законодательством Республики Казахстан, от государственных органов информацию о деятельности по противодействию коррупции',
                'участвуют в формировании антикоррупционной культуры'
            ],
            correctAnswer: 'проводят расследования по вопросам противодействия коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Физические лица, общественные объединения и иные юридические лица при противодействии коррупции НЕ применяют следующие меры:',
            choices: [
                'совершенствуют формы и методы борьбы с коррупционными преступлениями и правонарушениями, определяют стратегию и тактику оперативно-розыскной деятельности, вырабатывают и реализовывают меры по повышению ее эффективности',
                'осуществляют взаимодействие с другими субъектами противодействия коррупции и уполномоченным органом по противодействию коррупции',
                'проводят разъяснительную работу в средствах массовой информации и организуют социально значимые мероприятия по вопросам противодействия коррупции',
                'проводят исследования, в том числе научные и социологические, по вопросам противодействия коррупции'
            ],
            correctAnswer: 'совершенствуют формы и методы борьбы с коррупционными преступлениями и правонарушениями, определяют стратегию и тактику оперативно-розыскной деятельности, вырабатывают и реализовывают меры по повышению ее эффективности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционная служба не вправе:',
            choices: [
                'отстранять от должности госслужащего',
                'проводить анализ практики оперативно-розыскной и следственной деятельности, досудебного расследования по коррупционным преступлениям',
                'по имеющимся в производстве уголовным делам подвергать приводу лиц, уклоняющихся от явки по вызову',
                'конвоировать задержанных и лиц, заключенных под стражу'
            ],
            correctAnswer: 'отстранять от должности госслужащего'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицам, уполномоченным на выполнение государственных функций и лицам, приравненным к ним, запрещается?',
            choices: [
                'заключать гражданско-правовые сделки под псевдонимом',
                'заключать гражданско-правовые сделки под своим именем',
                'заключать договора купли-продажи недвижимости',
                'сдавать в имущественный наем (аренду) жилище, принадлежащее им на праве собственности, и получать доход от такой сдачи'
            ],
            correctAnswer: 'заключать гражданско-правовые сделки под псевдонимом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Несоблюдение антикоррупционных ограничений может быть основанием для чего?',
            choices: [
                'прекращения государственной службы или иной соответствующей деятельности',
                'перевода государственного служащего на иную должность',
                'наложения дисциплинарного взыскания на государственного служащего',
                'понижения государственного служащего в должности'
            ],
            correctAnswer: 'прекращения государственной службы или иной соответствующей деятельности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Деятельность, совместимая с выполнением государственных функций?',
            choices: [
                'педагогическая, научная и иная творческая',
                'предпринимательская деятельность',
                'участвовать в управлении хозяйствующим субъектом',
                'заниматься другой оплачиваемой деятельностью'
            ],
            correctAnswer: 'педагогическая, научная и иная творческая'
        },
        {
            type: EQuestionType.Radio,
            title: 'Является принципом противодействия коррупции?',
            choices: [
                'равная оплата труда',
                'этичность и профессионализм',
                'казахстанский патриотизм',
                'гласность и прозрачность'
            ],
            correctAnswer: 'гласность и прозрачность'
        },
        {
            type: EQuestionType.Radio,
            title: 'Мера ответственности за разглашение сведений, составляющих служебную тайну, не содержащее признаков уголовно наказуемого деяния, влечет:',
            choices: [
                'ответственность в соответствии с законами Республики Казахстан',
                'поощрение виновного лица',
                'понижение в должности виновного лица',
                'предупреждение о неполном служебном соответствии виновного лица'
            ],
            correctAnswer: 'ответственность в соответствии с законами Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'Физические и юридические лица, которые участвуют в выполнении функций по управлению государственным имуществом, представляют отчёты обо всех сделках имущественного характера и финансовой деятельности, связанных с государственной собственностью:',
            choices: [
                'в государственный орган по борьбе с коррупцией',
                'не предоставляют отчеты',
                'в государственный орган, осуществляющий в отношении государственного имущества правомочия собственника',
                'в налоговый орган по месту жительства'
            ],
            correctAnswer: 'в государственный орган, осуществляющий в отношении государственного имущества правомочия собственника'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицо, занимающее государственную должность, за непредставление декларации, если в содеянном не содержится признаков уголовно наказуемого деяния, может быть:',
            choices: [
                'привлечено к дисциплинарной ответственности',
                'наказано штрафными санкциями',
                'переведено на нижестоящую должность',
                'привлечено к уголовной ответственности'
            ],
            correctAnswer: 'привлечено к дисциплинарной ответственности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Правонарушения, связанные с коррупцией, могут быть совершены:',
            choices: [
                'бездействием',
                'действием и бездействием',
                'действием',
                'письменно'
            ],
            correctAnswer: 'действием и бездействием'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сведения, составляющие служебную тайну, представляются',
            choices: [
                'уполномоченному органу по финансовому мониторингу',
                'уполномоченному органу по противодействию коррупции',
                'Министерству юстиции',
                'всем государственным органам по соответствующему запросу'
            ],
            correctAnswer: 'уполномоченному органу по финансовому мониторингу'
        },
        {
            type: EQuestionType.Radio,
            title: 'Правонарушения, связанные с коррупцией, НЕ влекут:',
            choices: [
                'дисциплинарную ответственность',
                'административную ответственность',
                'гражданско-правовую ответственность',
                'уголовную ответственность'
            ],
            correctAnswer: 'гражданско-правовую ответственность'
        },
        {
            type: EQuestionType.Radio,
            title: 'В каком правовом акте определяются основные принципы противодействия коррупции?',
            choices: [
                'Закон «О противодействии коррупции»',
                'Уголовный кодекс Республики Казахстан',
                'Закон «О государственной службе»',
                'Кодекс Республики Казахстан об административных правонарушениях'
            ],
            correctAnswer: 'Закон «О противодействии коррупции»'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, занимающие ответственную государственную должность - это лица, занимающие должности, устанавливаемые:',
            choices: [
                'Конституционными и иными законами',
                'Постановлением Парламента',
                'Конституцией, конституционными и иными законами',
                'только Конституцией'
            ],
            correctAnswer: 'Конституцией, конституционными и иными законами'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сфера действия Закона РК «О противодействии коррупции»:',
            choices: [
                'на территории РК в отношении только физических лиц',
                'на территории РК в отношении юридических лиц',
                'на территории РК в отношении всех физических и юридических лиц',
                'на территории государств, участников СНГ в отношении всех физических и юридических лиц, противоправно предоставляющих имущественные блага и преимущества должностным лицам'
            ],
            correctAnswer: 'на территории РК в отношении всех физических и юридических лиц'
        },
        {
            type: EQuestionType.Radio,
            title: 'К лицам, уполномоченным на выполнение государственных функций, приравниваются:',
            choices: [
                'лица, избранные на ответственные должности неправительственных организаций',
                'лица, избранные в общественные организации',
                'лица, избранные в органы местного самоуправления',
                'лица, избранные на ответственные должности частных компаний'
            ],
            correctAnswer: 'лица, избранные в органы местного самоуправления'
        },
        {
            type: EQuestionType.Radio,
            title: 'Кто из близких людей лица, занимающего государственную должность, в соответствии с Законом РК «О противодействии коррупции» обязан предоставить декларацию о доходах и имуществе в налоговые органы?',
            choices: [
                'дети',
                'супруг (супруга)',
                'родители',
                'родители, супруг (супруга), совершеннолетние дети'
            ],
            correctAnswer: 'супруг (супруга)'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, уволенные с государственной службы по отрицательным мотивам, представляют в орган государственных доходов декларацию:',
            choices: [
                'в течение трех лет после увольнения',
                'в течение двух лет после увольнения',
                'не предоставляют в дальнейшем',
                'только в год увольнения'
            ],
            correctAnswer: 'в течение трех лет после увольнения'
        },
        {
            type: EQuestionType.Radio,
            title: 'В случаях отказа добровольно сдать незаконно полученное имущество или оплатить государству его стоимость или стоимость незаконно полученных услуг в результате коррупционных правонарушений их взыскание осуществляется:',
            choices: [
                'принудительно на основании протокола об уголовном правонарушении',
                'на основании решения суда, вступившего в законную силу, по иску прокурора, органов государственных доходов либо других государственных органов и должностных лиц, уполномоченных на это законом',
                'на основании решения уполномоченного органа по антикоррупционной деятельности',
                'по решению налоговой службы'
            ],
            correctAnswer: 'на основании решения суда, вступившего в законную силу, по иску прокурора, органов государственных доходов либо других государственных органов и должностных лиц, уполномоченных на это законом'
        },
        {
            type: EQuestionType.Radio,
            title: 'На правоотношения, связанные с приобретением в собственность жилищ и строительных материалов для строительства жилищ в Республике Казахстан, меры финансового контроля (вопрос по старой редакции):',
            choices: [
                'распространяются',
                'нет верных ответов',
                'распространяются в установленных законом случаях',
                'не распространяются'
            ],
            correctAnswer: 'не распространяются'
        },
        {
            type: EQuestionType.Radio,
            title: 'Непринятие лицами, уполномоченными на выполнение государственных функций, антикоррупционных ограничений влечет:',
            choices: [
                'увольнение',
                'наложение штрафа',
                'объявление выговора',
                'заключение под стражу'
            ],
            correctAnswer: 'увольнение'
        },
        {
            type: EQuestionType.Radio,
            title: 'Во всех случаях незаконного обогащения лиц, уполномоченных на выполнение государственных функций, или лиц, приравненных к ним, в результате коррупционных правонарушений стоимость незаконно полученных услуг подлежит:',
            choices: [
                'взысканию в доход государства',
                'не более чем в месячный срок после их обнаружения перечислению в соответствующий налоговый орган',
                'не более чем в двухнедельный срок после их обнаружения перечислению в специальный государственный фонд',
                'не подлежит взысканию'
            ],
            correctAnswer: 'взысканию в доход государства'
        },
        {
            type: EQuestionType.Radio,
            title: 'Выявление, пресечение, раскрытие, расследование и предупреждение коррупционных правонарушений и привлечение лиц, виновных в их совершении, к ответственности в пределах своей компетенции осуществляются:',
            choices: [
                'органами прокуратуры, национальной безопасности',
                'органами внутренних дел, военной полиции, службой экономических расследований',
                'Пограничной службой КНБ',
                'все вышеуказанное'
            ],
            correctAnswer: 'все вышеуказанное'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицам, занимающим ответственную государственную должность, лицам, уполномоченным на выполнение государственных функций, должностным лицам запрещается:',
            choices: [
                'открывать и иметь счета (вклады) в иностранных банках, расположенных за пределами Республики Казахстан',
                'выезжать в иностранные страны',
                'отправлять детей на учебу в иностранные страны',
                'принимать приглашения в командировки'
            ],
            correctAnswer: 'открывать и иметь счета (вклады) в иностранных банках, расположенных за пределами Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'Должностным и иным лицам, уполномоченным на выполнение государственных функций, и лицам, приравненным к ним, самостоятельно участвовать в управлении хозяйствующим субъектом:',
            choices: [
                'разрешается',
                'запрещается, если управление или участие в управлении хозяйствующим субъектом не входит в его должностные обязанности в соответствии с законами Республики Казахстан',
                'запрещается',
                'нет верных ответов'
            ],
            correctAnswer: 'запрещается, если управление или участие в управлении хозяйствующим субъектом не входит в его должностные обязанности в соответствии с законами Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'К деятельности, несовместимой с выполнением государственных функций НЕ относится:',
            choices: [
                'управлять хозяйствующим субъектом',
                'заниматься предпринимательской деятельностью, за исключением приобретения и (или) реализации паев открытых и интервальных паевых инвестиционных фондов, облигаций на организованном рынке ценных бумаг, акций коммерческих организаций (простые акции в объеме, не превышающем пяти процентов от общего количества голосующих акций организаций) на организованном рынке ценных бумаг',
                'самостоятельно участвовать в управлении хозяйствующим субъектом, если управление или участие в управлении хозяйствующим субъектом не входит в их должностные обязанности в соответствии с законами Республики Казахстан',
                'заниматься другой оплачиваемой деятельностью, кроме педагогической, научной и иной творческой деятельности'
            ],
            correctAnswer: 'управлять хозяйствующим субъектом'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицо, сообщившее о факте коррупционного правонарушения или иным образом оказывающее содействие в противодействии коррупции:',
            choices: [
                'поощряется в порядке, установленном Правительством Республики Казахстан',
                'поощряется дополнительным оплачиваемым трудовым отпуском в количестве 10 дней',
                'поощряется в виде надбавки к заработной плате',
                'поощряется дополнительным оплачиваемым трудовым отпуском в количестве 15 дней'
            ],
            correctAnswer: 'поощряется в порядке, установленном Правительством Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'Запрещается приобретать паи инвестиционных фондов, облигации, акции коммерческих организаций:',
            choices: [
                'банковским работникам',
                'депутатам Парламента Республики Казахстан',
                'Председателю Национального Банка Республики Казахстан и его заместителям',
                'государственным служащим корпуса А'
            ],
            correctAnswer: 'Председателю Национального Банка Республики Казахстан и его заместителям'
        },
        {
            type: EQuestionType.Radio,
            title: 'Система мер противодействия коррупции включает формирование и публикацию:',
            choices: [
                'Национального отчета по противодействию коррупции',
                'Государственного доклада о противодействии коррупции',
                'Национального доклада о противодействии коррупции',
                'Государственного отчета по противодействию коррупции'
            ],
            correctAnswer: 'Национального доклада о противодействии коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Система мер противодействия коррупции включает:',
            choices: [
                'меры противодействия коррупции в сфере международных отношений',
                'формирование Антикоррупционного кодекса',
                'сообщение о коррупционных правонарушениях',
                'формирование и публикацию Национального обращения о борьбе с коррупцией'
            ],
            correctAnswer: 'сообщение о коррупционных правонарушениях'
        },
        {
            type: EQuestionType.Radio,
            title: 'Функцией уполномоченного органа по противодействию коррупции является:',
            choices: [
                'конфискация имущества, приобретенного на средства, добытые преступным путем',
                'внесение на рассмотрение Парламента Республики Казахстан рекомендации по минимизации и устранению причин и условий возникновения коррупции',
                'взаимодействие с другими государственными органами, физическими и юридическими лицами по основным направлениям деятельности уполномоченного органа по противодействию коррупции',
                'ежегодное представление Парламенту Республики Казахстан Национального доклада о противодействии коррупции'
            ],
            correctAnswer: 'взаимодействие с другими государственными органами, физическими и юридическими лицами по основным направлениям деятельности уполномоченного органа по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Функцией уполномоченного органа по противодействию коррупции НЕ является:',
            choices: [
                'взаимодействие с другими государственными органами, физическими и юридическими лицами по основным направлениям деятельности уполномоченного органа по противодействию коррупции',
                'выработка предложений по совершенствованию образовательных программ в сфере формирования антикоррупционной культуры',
                'взаимодействие с соответствующими органами иностранных государств по вопросам противодействия коррупции',
                'создание учебных центров по противодействию коррупции'
            ],
            correctAnswer: 'создание учебных центров по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'В целях осуществления мер финансового контроля лица, являющиеся кандидатами на государственную должность и их супруги, представляют:',
            choices: [
                'налоговую декларацию',
                'общую декларацию',
                'декларацию об имущественной состоятельности',
                'декларацию об активах и обязательствах'
            ],
            correctAnswer: 'декларацию об активах и обязательствах'
        },
        {
            type: EQuestionType.Radio,
            title: 'Коррупционный риск – это:',
            choices: [
                'правовые, административные и организационные меры, направленные на снижение коррупционных рисков, повышение доверия общества к деятельности государственных органов, и иные меры в соответствии с настоящим Законом',
                'имеющее признаки коррупции противоправное виновное деяние (действие или бездействие), за которое законом установлена административная или уголовная ответственность',
                'деятельность субъектов противодействия коррупции по изучению, выявлению, ограничению и устранению причин и условий, способствующих совершению коррупционных правонарушений, путем разработки и внедрения системы превентивных мер',
                'возможность возникновения причин и условий, способствующих совершению коррупционных правонарушений'
            ],
            correctAnswer: 'возможность возникновения причин и условий, способствующих совершению коррупционных правонарушений'
        },
        {
            type: EQuestionType.Radio,
            title: 'Порядок подготовки, внесения Национального доклада о противодействии коррупции Президенту Республики Казахстан и его опубликования утверждается:',
            choices: [
                'Президентом Республики Казахстан',
                'Правительством Республики Казахстан',
                'Сенатом Парламента РК',
                'Мажилисом Парламента РК'
            ],
            correctAnswer: 'Президентом Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'Уполномоченный орган по противодействию коррупции ежегодно формирует:',
            choices: [
                'Национальный доклад о противодействии коррупции',
                'Национальный антикоррупционный доклад',
                'Государственный антикоррупционный доклад',
                'Государственный доклад о противодействии коррупции'
            ],
            correctAnswer: 'Национальный доклад о противодействии коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'К антикоррупционным ограничениям относятся:',
            choices: [
                'недопустимость совместной службы (работы) близких родственников, друзей и знакомых',
                'осуществление деятельности, не совместимой с предпринимательской деятельностью',
                'принятие любых видов подарков',
                'использование служебной и иной информации, не подлежащей официальному распространению, в целях получения или извлечения имущественных и неимущественных благ и преимуществ'
            ],
            correctAnswer: 'использование служебной и иной информации, не подлежащей официальному распространению, в целях получения или извлечения имущественных и неимущественных благ и преимуществ'
        },
        {
            type: EQuestionType.Radio,
            title: 'К антикоррупционным ограничениям, принимаемым лицами, занимающими ответственную государственную должность, лицами, уполномоченными на выполнение государственных функций, НЕ относится:',
            choices: [
                'осуществление деятельности в пределах должностных полномочий',
                'осуществление деятельности, не совместимой с выполнением государственных функций',
                'недопустимости совместной службы (работы) близких родственников, супругов и свойственников',
                'использование служебной и иной информации, не подлежащей официальному распространению, в целях получения или извлечения имущественных и неимущественных благ и преимуществ'
            ],
            correctAnswer: 'осуществление деятельности в пределах должностных полномочий'
        },
        {
            type: EQuestionType.Radio,
            title: 'Административные ответственность и взыскание за административные коррупционные правонарушения предусмотрены:',
            choices: [
                'Кодексом РК об административных правонарушениях',
                'Уголовным кодексом РК',
                'Законом РК «О противодействии коррупции»',
                'Антикоррупционным кодексом РК'
            ],
            correctAnswer: 'Кодексом РК об административных правонарушениях'
        },
        {
            type: EQuestionType.Radio,
            title: 'Стандарты по предупреждению коррупции для субъектов предпринимательства могут разрабатываться и приниматься объединениями (ассоциациями, союзами):',
            choices: [
                'советов директоров',
                'государственных предприятий',
                'уполномоченных органов',
                'субъектов предпринимательства'
            ],
            correctAnswer: 'субъектов предпринимательства'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционная служба уполномоченного органа по противодействию коррупции в пределах своих полномочий вправе:',
            choices: [
                'выносить приговоры в отношении лиц, совершивших коррупционное правонарушение',
                'использовать изоляторы временного содержания для допросов и следственных экспериментов',
                'составлять протоколы об уголовных и административных правонарушениях',
                'конвоировать задержанных и лиц, заключенных под стражу'
            ],
            correctAnswer: 'конвоировать задержанных и лиц, заключенных под стражу'
        },
        {
            type: EQuestionType.Radio,
            title: 'Что такое антикоррупционные ограничения?',
            choices: [
                'ограничения, установленные настоящим Законом и направленные на предупреждение коррупционных правонарушений',
                'ограничения, установленные настоящим Законом и направленные на выявления коррупционных правонарушений',
                'имеющее признаки коррупции противоправное виновное деяние (действие или бездействие), за которое законом установлена административная или уголовная ответственность',
                'возможность возникновения причин и условий, способствующих совершению коррупционных правонарушений'
            ],
            correctAnswer: 'ограничения, установленные настоящим Законом и направленные на предупреждение коррупционных правонарушений'
        },
        {
            type: EQuestionType.Radio,
            title: 'Государственные органы, организации и субъекты квазигосударственного сектора осуществляют внутренний …',
            choices: [
                'анализ коррупционных рисков',
                'контроль коррупционных рисков',
                'мониторинг коррупционных рисков',
                'профилактику коррупционных рисков'
            ],
            correctAnswer: 'анализ коррупционных рисков'
        },
        {
            type: EQuestionType.Radio,
            title: 'Внутренний анализ коррупционных рисков осуществляется:',
            choices: [
                'государственными органами, организациями и субъектами квазигосударственного сектора',
                'только государственными органами и организациями',
                'только субъектами квазигосударственного сектора',
                'Правительством'
            ],
            correctAnswer: 'государственными органами, организациями и субъектами квазигосударственного сектора'
        },
        {
            type: EQuestionType.Radio,
            title: 'Непрерывный процесс воспитания и обучения, осуществляемый в целях нравственного, интеллектуального, культурного развития и формирования активной гражданской позиции неприятия коррупции личностью это:',
            choices: [
                'антикоррупционное образование',
                'формирование антикоррупционной культуры',
                'анализ коррупционных рисков (внешний и внутренний)',
                'предотвращение и разрешение конфликта интересов'
            ],
            correctAnswer: 'антикоррупционное образование'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционные стандарты разрабатываются?',
            choices: [
                'государственными органами, организациями и субъектами квазигосударственного сектора при участии общественности',
                'физическими и юридическими лицами',
                'государственными органами и предприятиями',
                'Правительством'
            ],
            correctAnswer: 'государственными органами, организациями и субъектами квазигосударственного сектора при участии общественности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Функцией уполномоченного органа по противодействию коррупции является:',
            choices: [
                'выявление причин и условий, способствующих совершению коррупционных правонарушений в деятельности государственных органов, организаций и субъектов квазигосударственного сектора',
                'использование следственных изоляторов, тюрем для допросов',
                'утверждение реестра коррупционных правонарушений',
                'ежемесячное представление Президенту Национального доклада о противодействии коррупции'
            ],
            correctAnswer: 'выявление причин и условий, способствующих совершению коррупционных правонарушений в деятельности государственных органов, организаций и субъектов квазигосударственного сектора'
        },
        {
            type: EQuestionType.Radio,
            title: 'Государственный орган, определяющий перечень отраженных в декларации сведений, подлежащих опубликованию?',
            choices: [
                'Министерство финансов',
                'Министерство юстиции',
                'Уполномоченный орган по противодействию коррупции',
                'Правительство'
            ],
            correctAnswer: 'Уполномоченный орган по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицам, занимающим ответственную государственную должность, лицам, уполномоченным на выполнение государственных функций, запрещается заниматься другой оплачиваемой деятельностью, кроме:',
            choices: [
                'предпринимательской и научной',
                'педагогической, научной и иной творческой',
                'только педагогической',
                'самостоятельно участвовать в управлении хозяйствующим субъектом'
            ],
            correctAnswer: 'педагогической, научной и иной творческой'
        },
        {
            type: EQuestionType.Radio,
            title: 'За коррупционное правонарушение лицо может быть привлечено к какой ответственности?',
            choices: [
                'административной или уголовной',
                'дисциплинарной или административной',
                'дисциплинарной или гражданской',
                'гражданско-правовой или уголовной'
            ],
            correctAnswer: 'административной или уголовной'
        },
        {
            type: EQuestionType.Radio,
            title: 'Внешний анализ коррупционных рисков осуществляется?',
            choices: [
                'Администрацией Президента',
                'Уполномоченным органом по противодействию коррупции',
                'Министерством внутренних дел',
                'Министерством иностранных дел'
            ],
            correctAnswer: 'Уполномоченным органом по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сделки, договоры, совершенные в результате коррупционных правонарушений, признаются судом:',
            choices: [
                'недействительными',
                'недействительными в части противоречащей законодательству',
                'действительными, с согласия всех сторон',
                'действительными с согласия стороны не нарушавшей законодательство'
            ],
            correctAnswer: 'недействительными'
        },
        {
            type: EQuestionType.Radio,
            title: 'Если имеется конфликт интересов, то должностным лицам:',
            choices: [
                'запрещается осуществлять должностные обязанности',
                'необходимо переутвердить должностные обязанности',
                'необходимо принять необходимые меры по сглаживанию конфликта интересов',
                'разрешается переложить исполнение обязанностей на подчиненного'
            ],
            correctAnswer: 'запрещается осуществлять должностные обязанности'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не являются субъектом правоотношений, связанных с коррупцией:',
            choices: [
                'государственные органы',
                'субъекты квазигосударственного сектора, общественные объединения',
                'иностранные граждане, проживающие вне территории Казахстана',
                'физические и юридические лица'
            ],
            correctAnswer: 'иностранные граждане, проживающие вне территории Казахстана'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционная служба – это:',
            choices: [
                'оперативно-следственное подразделение уполномоченного органа по противодействию коррупции',
                'специальный государственный орган при Правительстве',
                'оперативно-следственное подразделение Комитета национальной безопасности',
                'следственное подразделение Министерства внутренних дел'
            ],
            correctAnswer: 'оперативно-следственное подразделение уполномоченного органа по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не относится к системе мер противодействия коррупции:',
            choices: [
                'формирование антикоррупционного этикета',
                'анализ коррупционных рисков',
                'формирование антикоррупционной культуры',
                'антикоррупционный мониторинг'
            ],
            correctAnswer: 'формирование антикоррупционного этикета'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не относится к системе мер противодействия коррупции:',
            choices: [
                'формирование и публикация Национального доклада о противодействии коррупции.',
                'анализ коррупционных рисков',
                'мониторинг и контроль соблюдения служебной этики',
                'формирование, соблюдение антикоррупционных стандартов'
            ],
            correctAnswer: 'мониторинг и контроль соблюдения служебной этики'
        },
        {
            type: EQuestionType.Radio,
            title: 'Цель противодействия коррупции?',
            choices: [
                'устранение коррупции в обществе',
                'превенция преступлений',
                'нетерпимость к коррупции в обществе',
                'проведение профилактической работы с целью предупреждения коррупции'
            ],
            correctAnswer: 'устранение коррупции в обществе'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не относится к источникам антикоррупционного мониторинга?',
            choices: [
                'сведения неправительственных и международных организаций',
                'данные социологических опросов и публикации в СМИ',
                'декларации о доходах и имуществе',
                'правовая статистика и обращения физических, юридических лиц'
            ],
            correctAnswer: 'декларации о доходах и имуществе'
        },
        {
            type: EQuestionType.Radio,
            title: 'Не предоставляет декларацию об активах и обязательствах?',
            choices: [
                'кандидаты в депутаты Парламента Республики Казахстан и Маслихатов',
                'кандидаты в Президенты Республики Казахстан',
                'лица, занимающие ответственную государственную должность',
                'кандидаты в акимы городов районного значения, посёлков, сёл, сельских округов'
            ],
            correctAnswer: 'лица, занимающие ответственную государственную должность'
        },
        {
            type: EQuestionType.Radio,
            title: 'Мера ответственности за разглашение сведений, составляющих служебную тайну, не содержащее признаков уголовно наказуемого деяния, влечет:',
            choices: [
                'увольнение виновного лица (ответственность в соответствии с законами)',
                'поощрение виновного лица',
                'понижение в должности виновного лица',
                'предупреждение о неполном служебном соответствии виновного лица'
            ],
            correctAnswer: 'увольнение виновного лица (ответственность в соответствии с законами)'
        },
        {
            type: EQuestionType.Radio,
            title: 'Декларацию о доходах и имуществе предоставляют?',
            choices: [
                'лица, уполномоченные на выполнение государственных функций и их супруги',
                'депутаты Парламента Республики Казахстан',
                'члены выборных органов местного самоуправления и их супруги',
                'акимы городов районного значения, поселков, сел, сельских округов'
            ],
            correctAnswer: 'лица, уполномоченные на выполнение государственных функций и их супруги'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционные стандарты – это?',
            choices: [
                'установленная для обособленной сферы общественных отношений система рекомендаций, направленная на предупреждение коррупции',
                'выявление и изучение причин и условий, способствующих совершению коррупционных правонарушений',
                'деятельность субъектов противодействия коррупции по сбору, обработке, обобщению, анализу и оценке информации, касающейся эффективности антикоррупционной политики, состояния правоприменительной практики в сфере противодействия коррупции, а также восприятия и оценки уровня коррупции обществом',
                'деятельность, осуществляемая субъектами противодействия коррупции в пределах своей компетенции по сохранению и укреплению в обществе системы ценностей, отражающей нетерпимость к коррупции'
            ],
            correctAnswer: 'установленная для обособленной сферы общественных отношений система рекомендаций, направленная на предупреждение коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Обязанностью должностного лица при отказе сотрудника, выполнявшего государственные функции и уволенного за совершение коррупционного правонарушения, сдать незаконно полученное имущество является?',
            choices: [
                'обратиться в суд с соответствующим иском',
                'обратиться в вышестоящий государственный орган',
                'обратиться в Министерство финансов',
                'обратиться в налоговые органы'
            ],
            correctAnswer: 'обратиться в суд с соответствующим иском'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционный мониторинг не распространяется на деятельность?',
            choices: [
                'субъектов квазигосударственного сектора',
                'государственных органов и организаций',
                'учреждений сферы образования',
                'специальных государственных органов'
            ],
            correctAnswer: 'специальных государственных органов'
        },
        {
            type: EQuestionType.Radio,
            title: 'Должностные лица НЕ могут занимать должности, находящиеся в непосредственной подчиненности должностям, занимаемым их близкими родственниками и супругом, а также?',
            choices: [
                'свойственниками',
                'друзьями',
                'попечителем',
                'опекуном'
            ],
            correctAnswer: 'свойственниками'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лицо, занимающее ответственную государственную должность – это:',
            choices: [
                'лицо, постоянно, временно или по специальному полномочию осуществляющее функции представителя власти либо выполняющее организационно-распорядительные или административно-хозяйственные функции в государственных органах, органах местного самоуправления, а также в Вооруженных Силах, других войсках и воинских формированиях Республики Казахстан',
                'лицо, занимающее должность, которая установлена Конституцией Республики Казахстан, конституционными и иными законами Республики Казахстан для непосредственного исполнения функций государства и полномочий государственных органов, в том числе депутат Парламента Республики Казахстан, судья, а равно лицо, занимающее согласно законодательству Республики Казахстан о государственной службе политическую государственную должность либо административную государственную должность корпуса А',
                'лицо, постоянно, временно либо по специальному полномочию исполняющее организационно-распорядительные или административно-хозяйственные функции в организациях квазигосударственного сектора',
                'государственный служащий в соответствии с законами Республики Казахстан о государственной службе, депутат маслихата, а также лицо, временно исполняющее обязанности, предусмотренные государственной должностью, до назначения его на государственную службу'
            ],
            correctAnswer: 'лицо, занимающее должность, которая установлена Конституцией Республики Казахстан, конституционными и иными законами Республики Казахстан для непосредственного исполнения функций государства и полномочий государственных органов, в том числе депутат Парламента Республики Казахстан, судья, а равно лицо, занимающее согласно законодательству Республики Казахстан о государственной службе политическую государственную должность либо административную государственную должность корпуса А'
        },
        {
            type: EQuestionType.Radio,
            title: 'В целях осуществления мер финансового контроля, кандидаты в депутаты Парламента и маслихатов, и их супруги представляют',
            choices: [
                'декларацию об активах и обязательствах',
                'декларацию о доходах и имуществе',
                'декларацию о доходах и банковских счетах',
                'договор на доверительное управление имуществом'
            ],
            correctAnswer: 'декларацию об активах и обязательствах'
        },
        {
            type: EQuestionType.Radio,
            title: 'Антикоррупционные стандарты учитываются…',
            choices: [
                'при разработке закона и постановлений',
                'при разработке законодательства и в правоприменительной практике',
                'при профилактике коррупционных правонарушений',
                'при формировании антикоррупционной культуры посредством комплекса мер образовательного, информационного и организационного характера'
            ],
            correctAnswer: 'при разработке законодательства и в правоприменительной практике'
        },
        {
            type: EQuestionType.Radio,
            title: 'Физические и юридические лица, которые участвуют в выполнении функций по управлению государственным имуществом, представляют в порядке и сроки, ……… , отчеты обо всех сделках имущественного характера и финансовой деятельности, связанных с государственной собственностью, в государственный орган, осуществляющий в отношении государственного имущества правомочия собственника:',
            choices: [
                'установленные Правительством РК',
                'установленные Премьер-Министром',
                'установленные Президентом РК',
                'установленные Уполномоченным органом по противодействию коррупции'
            ],
            correctAnswer: 'установленные Правительством РК'
        },
        {
            type: EQuestionType.Radio,
            title: 'Мера противодействия коррупции, принимаемая физическими лицами, общественными объединениями и иными юридическими лицами:',
            choices: [
                'проводят анализ практики оперативно-розыскной и следственной деятельности, досудебного расследования по коррупционным преступлениям',
                'сообщают об известных им фактах совершения коррупционных правонарушений',
                'изучают и распространяют положительный опыт противодействия коррупции',
                'запрашивают у государственных органов, организаций и должностных лиц информацию и материалы, способствующие раскрытию коррупционных правонарушений'
            ],
            correctAnswer: 'сообщают об известных им фактах совершения коррупционных правонарушений'
        },
        {
            type: EQuestionType.Radio,
            title: 'Внешний анализ коррупционных рисков осуществляется Уполномоченным органом по противодействию коррупции и распространяется на отношения в сфере:',
            choices: [
                'правосудия',
                'образования',
                'оперативно-розыскной деятельности',
                'уголовно-исполнительной деятельности'
            ],
            correctAnswer: 'образования'
        },
        {
            type: EQuestionType.Radio,
            title: 'Закон Республики Казахстан «О противодействии коррупции» действует на территории Республики Казахстан в отношении:',
            choices: [
                'только юридических лиц',
                'только физических лиц',
                'государственного сектора',
                'физических и юридических лиц'
            ],
            correctAnswer: 'физических и юридических лиц'
        },
        {
            type: EQuestionType.Radio,
            title: 'К субъекту противодействия коррупции относится:',
            choices: [
                'Агентство Республики Казахстан по делам государственной службы',
                'Уполномоченный орган по противодействию коррупции',
                'Министерство по делам государственной службы',
                'Министерство внутренних дел'
            ],
            correctAnswer: 'Уполномоченный орган по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'К антикоррупционным ограничениям, принимаемым лицами, занимающими ответственную государственную должность, лицами, уполномоченными на выполнение государственных функций, НЕ относится:',
            choices: [
                'получение денежной доплаты к должностному окладу',
                'осуществление деятельности, не совместимой с выполнением государственных функций',
                'недопустимости совместной службы (работы) близких родственников, супругов и свойственников',
                'использование служебной и иной информации, не подлежащей официальному распространению, в целях получения или извлечения имущественных и неимущественных благ и преимуществ'
            ],
            correctAnswer: 'получение денежной доплаты к должностному окладу'
        },
        {
            type: EQuestionType.Radio,
            title: 'Дата введения в действие закона Республики Казахстан «О противодействии коррупции»:',
            choices: [
                'октябрь 2015 года',
                'ноябрь 2015 года',
                'сентябрь 2015 года',
                'декабрь 2015 года'
            ],
            correctAnswer: 'ноябрь 2015 года'
        },
        {
            type: EQuestionType.Radio,
            title: 'Деятельность субъектов противодействия коррупции в пределах своих полномочий по предупреждению коррупции называется:',
            choices: [
                'предупреждение коррупции',
                'противодействие коррупции',
                'профилактика коррупции',
                'борьба с коррупцией'
            ],
            correctAnswer: 'противодействие коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Что не входит в систему мер противодействия коррупции?',
            choices: [
                'антикоррупционный мониторинг',
                'финансовый контроль',
                'ведение контроля за системой обратной связи',
                'сообщение о коррупционных правонарушениях'
            ],
            correctAnswer: 'ведение контроля за системой обратной связи'
        },
        {
            type: EQuestionType.Radio,
            title: 'Субъекты предпринимательства при осуществлении своей деятельности принимают меры по:',
            choices: [
                'предотвращению конфликта интересов',
                'участию в государственных закупках',
                'соблюдению банковской тайны',
                'соблюдению кодекса этики государственных служащих'
            ],
            correctAnswer: 'предотвращению конфликта интересов'
        },
        {
            type: EQuestionType.Radio,
            title: 'Кто определяет типовой порядок проведения внутреннего анализа коррупционных рисков?',
            choices: [
                'Уполномоченный орган по противодействию коррупции',
                'Правительство Республики Казахстан',
                'Агентство по делам государственной службы',
                'Руководитель государственного органа'
            ],
            correctAnswer: 'Уполномоченный орган по противодействию коррупции'
        },
        {
            type: EQuestionType.Radio,
            title: 'Кто изымает или производит выемку документов, товаров, предметов или иного имущества в соответствии с уголовно-процессуальным законодательством и законодательством Республики Казахстан об административных правонарушениях:',
            choices: [
                'Уполномоченный орган по противодействию коррупции',
                'Комитет национальной безопасности',
                'Антикоррупционная служба',
                'Специальное подразделение ЗАП при Комитете национальной безопасности'
            ],
            correctAnswer: 'Антикоррупционная служба'
        },
        {
            type: EQuestionType.Radio,
            title: 'Поступающие в органы государственных доходов сведения:',
            choices: [
                'не являются охраняемой законом тайной',
                'являются охраняемой законом тайной в соответствии с законодательством',
                'являются государственным секретом в соответствии с законодательством',
                'являются конфиденциальной информацией'
            ],
            correctAnswer: 'являются охраняемой законом тайной в соответствии с законодательством'
        },
        {
            type: EQuestionType.Radio,
            title: 'Вправе ли Антикоррупционная служба по имеющимся в производстве уголовным делам подвергать приводу лиц, уклоняющихся от явки по вызову:',
            choices: [
                'вправе, в пределах своих полномочий',
                'не вправе',
                'вправе, по согласованию с прокурором',
                'вправе, только с санкции суда'
            ],
            correctAnswer: 'вправе, в пределах своих полномочий'
        },
        {
            type: EQuestionType.Radio,
            title: 'Какими полномочиями обладают сотрудники антикоррупционной службы при исполнении ими своих служебных обязанностей?',
            choices: [
                'обладают неограниченными полномочиями, установленными законом Республики Казахстан «О правоохранительной службе» и иными законами Республики Казахстан',
                'обладают полномочиями, установленными законом Республики Казахстан «О противодействии коррупции» и иными подзаконными актами',
                'обладают полномочиями, установленными кодексами и законами Республики Казахстан',
                'обладают полномочиями, установленными законом Республики Казахстан «О правоохранительной службе» и иными законами Республики Казахстан'
            ],
            correctAnswer: 'обладают полномочиями, установленными законом Республики Казахстан «О правоохранительной службе» и иными законами Республики Казахстан'
        },
        {
            type: EQuestionType.Radio,
            title: 'В целях осуществления мер финансового контроля лица, определенные настоящей статьей 11 Закона, представляют следующие декларации физических лиц:',
            choices: [
                'декларации физических лиц и юридических лиц',
                'декларацию об активах и обязательствах, декларацию о доходах и имуществе',
                'декларацию об активах и обязательствах',
                'декларацию об имеющейся недвижимости и имуществе'
            ],
            correctAnswer: 'декларацию об активах и обязательствах, декларацию о доходах и имуществе'
        },
        {
            type: EQuestionType.Radio,
            title: 'Административная ответственность и взыскание за административные коррупционные правонарушения предусмотрены в каком из нормативных актов?',
            choices: [
                'Уголовный кодекс РК',
                'закон «О противодействии коррупции»',
                'Кодекс РК об административных правонарушениях',
                'закон «О правоохранительной службе»'
            ],
            correctAnswer: 'Кодекс РК об административных правонарушениях'
        },
        {
            type: EQuestionType.Radio,
            title: 'Если имеется конфликт интересов, то допускается ли исполнение должностных обязанностей лицам, занимающим ответственную государственную должность, лицам, уполномоченным на выполнение государственных функций:',
            choices: [
                'допускается',
                'запрещается',
                'разрешается по согласованию с руководителем государственного органа',
                'разрешается по согласованию с Уполномоченным органом по противодействию коррупции'
            ],
            correctAnswer: 'запрещается'
        },
        {
            type: EQuestionType.Radio,
            title: 'Вправе ли антикоррупционная служба, в соответствии с законодательством Республики Казахстан, создавать и использовать информационные системы, обеспечивающие решение возложенных на нее задач, организовывать исследования в ходе досудебного расследования, производства по делам об административных правонарушениях в порядке, установленном законодательством:',
            choices: [
                'не вправе',
                'вправе в исключительных случаях',
                'вправе, в пределах своих полномочий',
                'вправе, но только с санкции суда'
            ],
            correctAnswer: 'вправе, в пределах своих полномочий'
        },
        {
            type: EQuestionType.Radio,
            title: 'К лицам, уполномоченным на выполнение государственных функций, не относятся:',
            choices: [
                'лица, осуществляющие техническое обслуживание и обеспечивающие функционирование государственного органа',
                'государственные служащие',
                'должностные лица',
                'лица, приравненные к лицам, уполномоченным на выполнение государственных функций'
            ],
            correctAnswer: 'лица, осуществляющие техническое обслуживание и обеспечивающие функционирование государственного органа'
        },
        {
            type: EQuestionType.Radio,
            title: 'Лица, являющиеся кандидатами на государственную должность либо должность, связанную с выполнением государственных или приравненных к ним функций, представляют:',
            choices: [
                'декларацию о доходах и имуществе',
                'декларацию о правах',
                'общую декларацию',
                'декларацию об активах и обязательствах'
            ],
            correctAnswer: 'декларацию об активах и обязательствах'
        },
        {
            type: EQuestionType.Radio,
            title: 'Сведения о размерах и об источниках доходов должностных лиц, занимающих ответственные государственные должности (старая редакция закона):',
            choices: [
                'не могут быть опубликованы',
                'должны быть опубликованы в обязательном порядке',
                'могут быть опубликованы в порядке, установленном законодательством',
                'являются государственным секретом'
            ],
            correctAnswer: 'могут быть опубликованы в порядке, установленном законодательством'
        },
        {
            type: EQuestionType.Radio,
            title: 'Должностные лица принимают на себя антикоррупционные ограничения по:',
            choices: [
                'работе в строгом соответствии с должностными обязанностями',
                'принятию подарков в связи с исполнением служебных полномочий',
                'недопустимости совместной службы (работы) с друзьями и одноклассниками',
                'антикоррупционному противодействию единой автоматизированной деятельности'
            ],
            correctAnswer: 'принятию подарков в связи с исполнением служебных полномочий'
        },
        {
            type: EQuestionType.Radio,
            title: 'За пределами Республики Казахстан Закон «О противодействии коррупции»:',
            choices: [
                'действует в отношении граждан Республики Казахстан и юридических лиц, зарегистрированных в Республике Казахстан, если иное не предусмотрено международным договором, ратифицированным Республикой Казахстан',
                'не действует',
                'действует в отношении иностранных граждан',
                'действует в отношении иностранных граждан и юридических лиц'
            ],
            correctAnswer: 'действует в отношении граждан Республики Казахстан и юридических лиц, зарегистрированных в Республике Казахстан, если иное не предусмотрено международным договором, ратифицированным Республикой Казахстан'
        }
    ],
    time: 10,
    passingScore: 5,
    questionsTotal: 10
};
